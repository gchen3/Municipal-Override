...
-   Failed overrides have a small, insignificant association in the active ordered probit models.
-   These estimates use controls, Mundlak means, year indicators, and municipality-clustered standard errors.

\input{../outputs/tables/slides/northeast_moodys_main.tex}

## Override Frequency and Moody's Ratings

-   Three-year cumulative frequency estimates are negative for attempts, successes, and failures.
-   The estimates are small and insignificant, so the models do not provide clear support for H3b.
-   Estimates use controls, Mundlak means, year indicators, and municipality-clustered standard errors.

\input{../outputs/tables/slides/northeast_moodys_frequency.tex}
...
\input{../outputs/tables/slides/northeast_vote_share_annual.tex}

## Three-Year Override Frequency and Yes Votes

-   Three-year cumulative attempts are negative but insignificant.
...
\input{../outputs/tables/slides/northeast_vote_share_cumulative.tex}
...
\input{../outputs/tables/slides/northeast_repeated_event_counts.tex}

## Repeated-Event DiD: Downgrades

-   Attempt and success event-year downgrade estimates are positive but insignificant.
-   Failed-event downgrade estimates are negative in years $h=1$ and $h=2$.
-   Estimates are relative to $h=-1$ within each stack.

\input{../outputs/tables/slides/northeast_repeated_event_did_downgrade.tex}

## Repeated-Event DiD: Upgrades

-   Upgrade estimates are generally negative for attempts and successful overrides.
-   Event-year estimates for attempts and successes are statistically significant.
-   Estimates are relative to $h=-1$ within each stack.

\input{../outputs/tables/slides/northeast_repeated_event_did_upgrade.tex}

## Event Study: Override Attempts

\vspace{-0.15cm}

-   Upgrade probability falls in the event year; downgrade estimates are insignificant.
...
-   Upgrade probability falls in the event year; downgrade estimates remain insignificant.
