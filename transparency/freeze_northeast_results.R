...
if (length(args) != 1L || !nzchar(args[[1]])) {
  stop("Usage: Rscript --vanilla transparency/freeze_northeast_results.R <freeze_id>", call. = FALSE)
}
...
if (length(existing_outputs) == 0L) {
  stop("No expected Northeast outputs were found to freeze.", call. = FALSE)
}
if (length(missing_headline) == length(headline_outputs)) {
  stop("All expected headline outputs are missing; not creating an empty freeze bundle.", call. = FALSE)
}
...
message("Frozen Northeast outputs to ", bundle_dir)
