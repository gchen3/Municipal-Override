...
> **Status: implemented.** The Stata-replication track shipped as `R/00`–`R/09`, matching the script names below. See `README.md` for as-built details.
...
> The repeated-event DiD extension is documented separately in `docs/did_repeated_event_design_plan.md` and is not part of this Stata-replication plan. (Earlier drafts listed DiD scripts as items 10–16 here; that content has moved to the dedicated DiD plan to avoid duplication.)
