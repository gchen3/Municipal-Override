...
> **Status: implemented.** Active outputs are generated by `R/run_active_workflow.R`; slide assets ship via `R/13_prepare_slide_tables.R` and `R/14_prepare_slide_figures.R` (not the `R/15`/`R/16` names used below), and slide tables are `outputs/tables/slides/northeast_*.tex`. See `README.md` for as-built script and output names.
